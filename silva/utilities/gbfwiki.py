...
class Wiki:
    ...
    async def _get_wikimedia_query_api(
            self, cmpageid: int) -> Dict[str, str]:
        ...
        url = self.url
        async with aiohttp.ClientSession() as session:
            async_res = await session.get(url, headers=headers, params=params, timeout=10)
    ...
    async def get_summons_page(self):
        ...
        summons = [summon for summon in summons_full
                   if "Summons List" not in summon['title']]
        return summons
    ...
    async def get_page(self, pageid: int) -> Dict[str, str]:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                        self.url, params=params, headers=self.headers) as resp:
                    text = await resp.json()
            except Exception as e:
                raise(self.UnreachableWikiError(e))
    ...
    async def get_summon_first_half(self, page: BeautifulSoup) -> str:
        ...
        if not page.find(text='Combo Call Name'):
            raise self.NoPageFound(f'Summon "{char_name})" has no combo call information.')
