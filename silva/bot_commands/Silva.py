...
class Commands(commands.Cog, name="GBF-related commands"):
    ...
    async def calculate_spark(
        ...
        msg += f" you have {t.no_if_zero(crystals)} crystal{t.is_plural(crystals)},"
        msg += f" {t.no_if_zero(singles)} single-draw ticket{t.is_plural(singles)},"
        msg += f" and {t.no_if_zero(tens)} ten-draw ticket{t.is_plural(tens)}."
        msg += f" You have **{t.no_if_zero(draws)} roll{t.is_plural(draws)}**."
    ...
    @commands.command(
        name="costumesparkcalc",
        aliases=["costumecalc", "skincalc", "skinspark", "costume"],
    )
    async def calculate_skin_spark(self, ctx, crystals: int = 0):
        """
        Calculates how many draws you have and how close you are to a spark
        the new costume gacha. Presumably, this only takes crystals.
        :param crystals (int): the amount of crystals a player holds.
        ...
        msg = f"{ctx.author.display_name},"
        msg += f" you have {t.no_if_zero(crystals)} crystal{t.is_plural(crystals)}."
        msg += f" You have **{t.no_if_zero(draws)} costume gacha draw{t.is_plural(draws)}**."
        if spark_percentage >= 100 and spark_percentage < 200:
            msg += " You have one costume gacha spark and you're"
            msg += f" {(spark_percentage % 100):.2f}%"
            msg += " closer to a spark after."
        elif spark_percentage >= 200:
            msg += f" You have {int(spark_percentage // 100)} costume gacha sparks and"
            msg += f" you're {(spark_percentage % 100):.2f}% closer"
            msg += " to a spark after."
        else:
            msg += f" You are {spark_percentage:.2f}% closer"
    ...
    async def song(self, ctx):
        ...
        try:
            ...
            if status >= 400:
                ...
                new_text += (
                    f" {owner.mention}," " please check why Silva isn't working right.)"
                )
    ...
    async def events(self, ctx):
        ...
        try:
            ...
            await msg.edit(content="I couldn't retrieve the events at this time.")
    ...
    async def add_raid_role(self, ctx, *, role_name: str):
        ...
        guild = ctx.guild if ctx.guild else "a direct message"
        logging.info(
            f'add-my-role requested by {ctx.author} in {guild} to add role "{role_name}" to themselves.'
        )
        ...
        if role_name.lower() not in role_names:
            logging.warning(
                f"{ctx.author} tried to add {role_name} but that's not in the role database."
            )
    ...
    async def rm_raid_role(self, ctx, *, role_name: str):
        ...
        guild = ctx.guild if ctx.guild else "a direct message"
        logging.info(
            f'del-my-raid-role requested by {ctx.author} in {guild} to remove role "{role_name}" from themselves.'
        )
        roles = await self.db_utils.get_raid_roles()
        message = ctx.message
        role_names = [role["role_name"].lower() for role in roles]
        if role_name.lower() not in role_names:
            logging.warning(
                f"{ctx.author} tried to remove {role_name} from themselves, but that's not in the role database."
            )
            return await message.add_reaction("❌")
        role_id = [
            role["role_id"]
